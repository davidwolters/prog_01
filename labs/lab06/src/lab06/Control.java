...
import lab06.Message.MessageType;
...
class StartControl implements ActionListener {
	...
	final private JLabel connectLabel = new JLabel("   Press Connect to connect to another computer",
	...
	final private JLabel recordLabel = new JLabel("   Press Record to choose a file for recording the game",
			SwingConstants.RIGHT);
	final private JLabel playbackLabel = new JLabel("   Press Playback to playback a recorded game",
			SwingConstants.RIGHT);
	...
	final private String[] pebblesChoisesStr = { "Pebbles:  8", "Pebbles: 10", "Pebbles: 12", "Pebbles: 14",
			"Pebbles: 16" };
	...
	final private String welcomeStatus = "         Welcome to the game of Control!\n"
			...
			+ "Alternatively, to listen in case someone tries to connect to you " + "press the Listen Button.";
	...
	final private String connectingStatus = "Please enter either the IP name or the number "
			...
			+ "in this field will make me start connecting to it. " + "Pressing Abort returns to main screen.";
	...
	final private String acceptingstatus = "An opponent at the address given above is ready to play.\n\n"
			...
			+ "If you do not wish to play this opponent press Abort " + "which returns you to the main menu";
	...
	final private static int DEFAULT_GRAPHICS = 1, DEFAULT_SHAPE = 3, DEFAULT_BOARDSHAPE = 1, DEFAULT_SPEED = 1,
			DEFAULT_WIDTH = 8, DEFAULT_HEIGHT = 8, DEFAULT_PEBBLES = 8, DEFAULT_LENGTH = 10;
	...
	StartControl(boolean testServer, boolean testClient) {
		...
		try // try to get previously saved game parameters from the file
			// options<V>.controloptions
		{
		...
		} // If it doesn't work then just ignore
	...
	private void options() {
		...
		Border typeBorder = new TitledBorder(new BevelBorder(BevelBorder.RAISED), "Game type", TitledBorder.ABOVE_TOP,
				TitledBorder.LEFT);
		typePanel.setBorder(typeBorder);

		ButtonGroup hillGroup = new ButtonGroup(); // Group of buttons for board topography
		if (withHills)
			hillsButton.setSelected(true);
		else
			noHillsButton.setSelected(true);
		...
		Border hillsBorder = new TitledBorder(new BevelBorder(BevelBorder.RAISED), "Board topography",
		...
		if (graphics == 1)
			compactButton.setSelected(true);
		else if (graphics == 2)
			largeButton.setSelected(true);
		else
			hugeButton.setSelected(true);
		...
		Border graphicBorder = new TitledBorder(new BevelBorder(BevelBorder.RAISED), "Graphics", TitledBorder.ABOVE_TOP,
				TitledBorder.LEFT);
		...
		if (boardShape == 1)
			boardSquareButton.setSelected(true);
		else if (boardShape == 2)
			boardCircleButton.setSelected(true);
		...
		Border boardShapeBorder = new TitledBorder(new BevelBorder(BevelBorder.RAISED), "Board shape",
				TitledBorder.ABOVE_TOP, TitledBorder.LEFT);
		...
		Border shapeBorder = new TitledBorder(new BevelBorder(BevelBorder.RAISED), "Shape of areas",
				TitledBorder.ABOVE_TOP, TitledBorder.LEFT);
		...
		Border speedBorder = new TitledBorder(new BevelBorder(BevelBorder.RAISED), "Game speed", TitledBorder.ABOVE_TOP,
				TitledBorder.LEFT);
		...
		sizePanel.setBorder(new TitledBorder(new BevelBorder(BevelBorder.RAISED), "Relative board dimensions",
				TitledBorder.ABOVE_TOP, TitledBorder.LEFT));
		...
		resourcePanel.setBorder(new TitledBorder(new BevelBorder(BevelBorder.RAISED), "Pebbles", TitledBorder.ABOVE_TOP,
				TitledBorder.LEFT));
		...
		lengthPanel.setBorder(new TitledBorder(new BevelBorder(BevelBorder.RAISED), "Game length",
				TitledBorder.ABOVE_TOP, TitledBorder.LEFT));
	...
	private void exitOptions() {
		...
		if (boardSquareButton.isSelected())
			boardShape = 1;
		else
			boardShape = 2;
	...
	private void playBack() {
		...
		if (myFileChooser.showOpenDialog(myFrame) == JFileChooser.APPROVE_OPTION)
			try {
				...
				if (!s.equals("Game of Control")) {
					reason = "Not a recorded game file";
					throw new Exception();
				} // check it begins correctly
				if (!s2.equals(Control.VERSION_NUMBER)) {
					reason = "Wrong version number of the recorded game";
					throw new Exception();
				...
				JOptionPane.showMessageDialog(thePane, "Sorry, unable to open and read from file.\n" + // tell user if
																										// and why it
																										// failed
						"The reason is:\n" + ex.toString() + "  " + reason, "File Error", JOptionPane.ERROR_MESSAGE);
				playback = null;
			}
	...
	private void tryConnecting() {
		...
		if (outSocket != null)
			...
			} // close any remaining outsocket
		try {
			outSocket = new Socket(lastAddress, PORT_NUMBER); // set up the connection
			outgoing = new ObjectOutputStream(outSocket.getOutputStream());
			incoming = new ObjectInputStream(outSocket.getInputStream());
		} catch (Exception e) {
			failed = true; // if setting up connection failed
	...
	private void isConnected() {
		...
		try {
			...
			outgoing.writeObject( // then tell my params
					new GameParameters(Control.VERSION_NUMBER, graphics, isLite, withHills, nick, shape, boardShape,
							speed, width, height, pebbles, length));
			...
			failed = true;
			reason = e.toString(); // if IOfailure remember why
		}
		if (failed) {
	...
	private class GetAccept extends Thread {
		...
		{
			try {
				reply = (String) incoming.readObject();
			} // try reading something from opponent
			catch (Exception e) {
				failed = true;
				reason = e.toString();
			} // if it failed remember why

			if (reason == null)
				reason = reply; // default fail reason is a text transmitted by opponent ( eg mismatch params)

			if (!aborted) // only continue if I should
				if (failed || !reply.equals("OK Game of control")) // if something amiss then inform user
				{
					JOptionPane.showMessageDialog(thePane,
				...
				{
					try {
						opponentNick = (String) incoming.readObject();
					} // read opponent's nick
					catch (Exception e) {
						failed = true;
						reason = e.toString();
					} // if that fails I just ignore it
	...
	// -------------------------------------------------------------------------------
	// All is ready, so start the game. We come here when getAccept invokes the
	// method
	...
	private void startGame() {
		...
		if (record != null) // if recording then write game params on the record
			try {
				record.writeObject(new RecordedGame(Control.VERSION_NUMBER, isLite, withHills, graphics,
						playWhite ? nick : opponentNick, playWhite ? opponentNick : nick, shape, boardShape, width,
						height, pebbles, length));
			} catch (Exception e) {
				JOptionPane.showMessageDialog(thePane, "Sorry, unable to record.\n" + // if this fail tell user
						"The reason  is:\n" + "no response for five seconds", "File Error", JOptionPane.ERROR_MESSAGE);
				record = null; // and cease trying to record
			}

		Player player = new Player(isLite, withHills, playWhite, controlTimer, incoming, outgoing, record, graphics,
				shape, boardShape, height, width, pebbles, speed, length); // Set up the player
	...
	{
		if (getAccept != null)
			getAccept.abort(); // abort any threads listening for communication
		...
		try {
			if (incoming != null)
				incoming.close(); // close all streams
			if (inSocket != null)
				inSocket.close();
			if (outgoing != null)
				outgoing.close();
	...
	private void startListening() {
		...
		buttonsPanel.add(quitPanel);
		if (ipLabel != null)
			buttonsPanel.add(ipLabel);

		JPanel statusPanel = new JPanel();
		TextArea statusArea = new TextArea(listenStatus, 7, 40, TextArea.SCROLLBARS_VERTICAL_ONLY); // message about
																									// listening
		statusArea.setEditable(false);
		statusPanel.add(statusArea);
	...
	// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	//
	// Inner class GetConnection: a thread which tries to establish connection on
	// the server side
	//
	// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...

	{
		...
		public void run() {
			...
			{
				try {
					inSocket = new ServerSocket(StartControl.PORT_NUMBER);
				} // create a server socket
				catch (Exception e) {
					failed = true; // if that does not work tell user

					JOptionPane.showMessageDialog(thePane,
							"Sorry, unable to open server\n"
...

			{
				try // try to set up connection
				{
					...
					if (!((String) incoming.readObject()).equals("Game of control")) // check if it is someone who wants
																						// to play
					{
						failed = true;
						reason = "someone made contact but not to play Control";
					}
					...
					if (!gp.versionNumber.equals(Control.VERSION_NUMBER)) {
						failed = true;
						reason = "Wrong version of the game";
					} // check that we all agree on game parameters
					if (gp.graphics != graphics) {
						failed = true;
						reason = "Different graphic sizes";
					}
					if (gp.shape != shape) {
						failed = true;
						reason = "Mismatching area shapes";
					}
					if (gp.boardShape != boardShape) {
						failed = true;
						reason = "Mismatching board shape";
					}
					if (gp.isLite != isLite) {
						failed = true;
						reason = "Only one of us wants ControLITE";
					}
					if (gp.withHills != withHills) {
						failed = true;
						reason = "Only one of us wants to play with hills";
					}
					if (gp.width != width) {
						failed = true;
						reason = "Mismatching board width";
					}
					if (gp.height != height) {
						failed = true;
						reason = "Mismatching board height";
					}
					if (gp.pebbles != pebbles) {
						failed = true;
						reason = "Mismatching number of pebbles";
					}
					if (gp.speed != speed) {
						failed = true;
						reason = "Mismatching game speed";
					}
					if (gp.length != length) {
						failed = true;
						reason = "Mismatching game length";
					}

					Object o = incoming.readObject(); // opponent decides who should play white

					if (((String) o).equals("I play white"))
						playWhite = false;
					else if (((String) o).equals("You play white"))
						playWhite = true;
					else {
						failed = true;
						reason = "someone contacted me but then quit";
					}

				} catch (Exception e) {
					failed = true;
					reason = e.toString();
				} // if IO failed remember why

				if (failed & !aborted) // if failed tell opponent why
				{
					JOptionPane.showMessageDialog(thePane,
							"Incoming connection refused\n"
									+ "Someone tried to make contact but we could not start the game.\n"
									+ "The reason is:\n" + reason,
							"Game Error", JOptionPane.ERROR_MESSAGE);
					try {
						outgoing.writeObject(reason);
					} catch (Exception e) {
					} finally {
						abortIt();
					}
	...
	// -------------------------------------------------------------------------------
	// Screen where user decides whether to play an opponent. We come here when
	// getConnection
...

	{
		...
		if (opponentNick.equals(""))
			hostLabel = new JLabel("Anonymous opponent wants to play from " + hostname, SwingConstants.CENTER); // label
																												// displaying
																												// the
																												// name
		else
			hostLabel = new JLabel(opponentNick + " wants to play from  " + hostname, SwingConstants.CENTER); // label
																												// displaying
																												// the
																												// name
	...
	private void playGame() {
		...
		try {
			outgoing.writeObject("OK Game of control");
			outgoing.writeObject(nick);
		} // tell opponent I am willing to play, and my nick
		catch (Exception e) { // Oops, opponent dropped out
			failed = true;
			JOptionPane.showMessageDialog(thePane,
		...
		if (!failed)
			startGame(); // all is well so game starts
	}
...
class GameParameters implements Serializable {
	...
	public GameParameters(String v, int gr, boolean il, boolean wH, String n, int g, int bS, int s, int w, int h, int p,
			int l) {
		versionNumber = v;
		graphics = gr;
		isLite = il;
...

{
	...
	public RecordedGame(String v, boolean il, boolean wH, int gr, String w, String b, int g, int bS, int wi, int h,
			int p, int l) {
		versionNumber = v;
		isLite = il;
		withHills = wH;
...

{
	...
	private static final Color BACKGROUNDCOLOR = new Color(160, 180, 120); // Background color for the timer (currently
																			// same as for board)
	...
	public void paintComponent(Graphics page) {
		...
		page.fillRect(LEFT_OFFSET, TOP_OFFSET, (int) (remainingFraction * BAR_LENGTH), BAR_HEIGHT);
	}
...
//-----------------------------------------------------------------------
	...
	{
		remainingFraction = 1 - (float) (System.currentTimeMillis() - startTime) / (float) totalTime; // Fraction of
																										// total time
																										// that remains
		if (remainingFraction < 0)
			remainingFraction = (float) 0; // (should neve be negative)
		...
		if (remainingFraction <= 0) { // If finished then stop timer and notify player
			...
			repaint(); // OW just repaint the timer bar
 ...
 ************************************************************************************************
 * +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 * +++++++++++++++++ Create the graphic interface for a player: set up a frame
 * with panels
 * +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 * +++++++++++++++++
 ***********************************************************************************************
...

{
			...
			String opponentNick) // nick the opponent uses
	{
		...
		cheatListener = new CheatListener(this);
		...
		bottoms.add(Box.createRigidArea(new Dimension(0, ControlTimer.BAR_HEIGHT))); // (ow the layout manager will
																						// reserve no space)
		bottoms.add(timer);
		...
		player.addKeyListener(cheatListener);
		...
		{
			whiteNick = nick;
			blackNick = opponentNick;
		} else {
			whiteNick = opponentNick;
			blackNick = nick;
		}
	...
	public void cheatEntered() {
		player.sendCheatMessage();
	}

	// ------------------------------------------------------------------------
	// Strangely, this is necessary in order to let the player listen for key
	// actions
	// ------------------------------------------------------------------------
 ...
 *
 * Serves one player with game logics Reacts to player mouse commands , sends
 * moves to opponent, and reacts to to moves received by opponent
 *
 * +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 * +++++++++++++++++++++++++++++++++++
 *
...
//------------------------------------------------------------------------------------------------------------
	...
	Player(boolean isLite, // true if playing controlite
			...
			ObjectOutputStream outgoing, ObjectOutputStream record, // if not null then the stream where games are
																	// recorded
...

	{
		...
		board = new Board(isLite, withHills, graphics, shape, boardShape, height, width, pebbles, playwhite); // get me
																												// a
																												// board
		...
		add(Box.createRigidArea(new Dimension(board.SIZE * (board.WIDTH + 2) - 10, // crazy, but need to tell layout
																					// manager how big I am
				board.SIZE * (board.HEIGHT + 2) - 10)));

		mutex = new Semaphore(1); // Critical sections protects updates of game state

		ControlListener myListener = new ControlListener(); // Add my inner class to the listeners (gets moves from
															// mouse)
	...
	// --------------------------------------------------------------------
	// Method to accept identity of my interface (only done once, in the setup
	// phase)
	...
	void setInterface(PlayerInterface i) {
		...
		if (withHills & playWhite) // if hills are used and I play white
		{
			...
			try {
				outgoing.writeObject(hills);
			} // send them to opponent
	...
	public void paintComponent(Graphics g) {
		...
		board.paint(g, // Draw the board
				...
				(ackedNumber != messageNumber - 1 & !waiting) & !board.allVisible, // Show stall if oneSecondTimer has
																					// expired and unacked messages,
		...
		if (currentSelection != null && currentPiece == null) // Highlight current selector if any
		{
			...
			if (!waiting && currentPos != null && board.isLegal(currentPos) && currentSelection.canBuild(currentPos)) // Highlight
																														// mouse
																														// position
																														// if
																														// build
	...
	// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	//
	// Game methods. Note that before calling any of these, the caller must first
	// claim the mutex lock.
	// That way, there will be no races between the thread that listens to player
	// moves through the mouse
	// and the thread that listens through opponent moves through the incoming
	// stream.
	//
	// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	public synchronized void sendMessage(Message message) // actually "synchronized" is probably not needed here bc of
															// the mutex
	{
	...
	public void sendCheatMessage(int number) {

				sendMessage(
								new Message(MessageType.CHEAT, clock, number, null, null, null, playWhite, ""));
	}
	...
	// -----------------------------------------------------------------------------------
	// SendMove: builds a message out of the info in a move, and sends it using
	// sendMessage
	// -----------------------------------------------------------------------------------
	...
	// **************************************************************************
	//
	// Methods to serve my user in making moves: determining effects of mouse
	// actions
	//
	// **************************************************************************
	...
	// -----------------------------------------------------------------
	// appraise: check if someone has won or if it is a draw. If so report it to my
	// interface
	// -----------------------------------------------------------------
	...
	// -----------------------------------------------------------------
	// interrupt from game clock oneSecondTimer. Just update clock and repaint.
	...
	void endGame(String recordReason) // parameter is what will go on a recording of the game
	{
		...
		recordMessage(new Message(Message.MessageType.QUIT, clock, messageNumber, // Here is where quit messages are
																					// recorded
	...
	{
		sendMessage(new Message(Message.MessageType.QUIT, clock, messageNumber, null, null, null, playWhite, "")); // Tell
																													// opponent
																													// I
																													// resigned
	...
	private class ControlListener implements MouseListener, MouseMotionListener, KeyListener {
...

		{
			...
			{
				if (!waiting && !clockHasExpired && ackedNumber == messageNumber - 1 && board.isLegal(p) // either a
																											// build
																											// attempt
		...
		public void mouseDragged(MouseEvent event) {
			...
			if (!board.isLegal(p)) // if outside board (this is for debugging)
			{
				...
				repaint();
			} // repaint, to show outline has disappeared
			...
			if (!clockHasExpired && !waiting && ackedNumber == messageNumber - 1) // if allowed to make a move
			{
				boolean rep = (!p.equals(currentPos) // and mouse coordinates have changed
						...
						&& (currentSelection.canBuild(p)
								|| (currentPos != null && currentSelection.canBuild(currentPos)))); // or I could build
																									// it at old
																									// position
				currentPos = p; // in any case note the new position
				if (rep) {
					repaint();
				} // repaint only necessary under conditions above
			} else {
				currentPos = p;
	...
	//
	// Inner class:
	// Listens for moves sent by opponent and forwards them to the player through
	// getMove.
	//
	// *********************************************************************************
	...
	private class MoveListener extends Thread // will listen in a separate thread
	{
		...
		// -----------------------------------------------------------------------------------------
		// Main loop: Ad infinitum, listen for messages from oppponent and report to
		// player
		// -----------------------------------------------------------------------------------------
		...
		public void run() {
			if (withHills & !playWhite) // If hills are used and I play black
				...
				{
					Board.Hill[] hills = (Board.Hill[]) incoming.readObject(); // read them from the stream from
																				// opponent
			...
			while (!fail & !resigned & !abort) // do forever until someone stops me
			{
...

				{
					...
					case EXPIRED: // Opponent's clock has expired
					{
						...
						if (board.relativeNumberOfControlledSquares > 0) // then report outcome depending on who
																			// controls most squares
...
class Message implements Serializable {
	enum MessageType {
		ACK, MOVE, BUILD, QUIT, EXPIRED, CHEAT
	...
	public Message(MessageType mt, int ti, int nu, Board.Position s, Board.Position p, Piece.Type pt, boolean w,
			String te) {
		messageType = mt;
		time = ti;
		number = nu;
...

{
			...
			ObjectInputStream playback) // from where I get the game
	{
		...
		try {
			rg = (RecordedGame) playback.readObject();
		} // get game parameters
		catch (Exception e) {
			JOptionPane.showMessageDialog(this, "Not a recorded game\n" + // if impossible tell user and exit
					"The reason is:\n" + e.toString(), "Recording ended ", JOptionPane.INFORMATION_MESSAGE);
			failed = true;
		}
		...
		if (!failed) // if game parameters are OK:
		{
			...
			if ((rg.whiteNick.equals("")) & (rg.blackNick.equals("")))
				noids = true;
			else
				noids = false; // set noids and nicks for white and black
			if (rg.whiteNick.equals(""))
				whiteNick = "White";
			else
				whiteNick = rg.whiteNick;
			if (rg.blackNick.equals(""))
				blackNick = "Black";
			else
				blackNick = rg.blackNick;
			...
			startControl.playAgain(); // if failed to read game parameters, let the guy who called me know
	} // end of constructor
...
class Replayer extends JPanel implements ActionListener {
	...
	// -----------------------------------------------------------------------------
	// Playmoves: executes every second. Reads a recorded move if necessary, and
	// replays it
	// -----------------------------------------------------------------------------
...

	{

		{
			if (message == null) // if I have no current move
				try {
					message = (Message) playback.readObject();
				} // then read one
				catch (Exception e) {
					aborted = true;
					reason = e.toString();
				} // if that does not work I must exit
	...
	private void playBackMove(Message message) // message is the encoded record of one move
	{
		...
		case MOVE:
			playMove();
			break; // an ordinary move displayed by playMove
		case BUILD:
			playBuild();
			break; // a build displayed by playBuild
		case QUIT:
			terminated = true; // a quit: tell my parent and stop playback
			parent.endGame(message.text);
			break;
		case EXPIRED:
			oneSecondTimer.setDelay(1);
			oneSecondTimer.start();
			break; // EXPIRED message, just get the next one

		default:
			message = null; // no recognizable move: tell parent
	...
	private void playMove3() {
		...
		if (ffw & !terminated & !aborted)
			playMoves(); // if on fast forward go immediately to play next move
		else
			sleepAWhile((int) (System.currentTimeMillis() - startTime)); // else wait what is left of the second
	}
	...
	{
		outlinePiece = board.newPiece(message.pt, message.white, message.white, null); // the new piece is first present
																						// as an outline
		if (message.pt == Piece.Type.PEBBLE)
			board.decreaseRemaining(isWhite); // needed since we created the pebble at -1,-1 (ow board would do this
												// automatically)
		outline = destPos;
		board.generateBoardGraphics(currentPiece);
		repaint(); // repaint
...
class Board {
	...
	boolean allVisible = true; // true if board should be displayed without fog of war
	...
	private static final int numberOfSamples = 50; // Sqrt of number of sample points for determining who controls most
													// area
	...
	private static final Font myFont = new Font("SansSerif", Font.BOLD, 24); // Font for displaying number of controlled
																				// squares
	...
	private int numberOfRegions, numberOfBox, numberOfIntersection, numberOfUncontested, // debug tracers, some
																							// currently ununsed
			timeCalculate, timeHighlight, timeBuildable, timeGraphics, timeInit, timeMy, timeOpponents;
	...
	static class Hill implements Serializable // must be serializable: sent to opponent and written on recording
	{
		...
		{
			boolean r = false;
			if (!boardArea.contains(pos.x, pos.y, diameter, diameter)) // If the board are does not contain the bounding
																		// box, then true
				return true;
			for (Hill otherHill : hills) // Ow, for all hills generated so far
			{
				if (otherHill != null & otherHill != this)
					r |= otherHill.center.distance(center) // Check that the distance between centers is larger than
															// hill extents
		...
		} // p is closer than radius to center
	...
	Board(boolean isLite, boolean withHills, int graphics, int shape, int boardS, int height, // params are board
																								// dimensions
			int width, int pebbles, boolean iPlayWhite) // and initial number of pebbles
	{
		...
		if (graphics == 1)
			SIZE = 60; // set the SIZE according to graphics parameter
		else if (graphics == 2)
			SIZE = 80;
		else
			SIZE = 100;
		...
		whiteBuild = new BufferedImage(10, 10, BufferedImage.TYPE_INT_RGB);
...
//-------------- Texture for buildableArea controlled by black
		...
		blackBuild = new BufferedImage(10, 10, BufferedImage.TYPE_INT_RGB);
		...
		// -------------- The buffered image for the board (note it includes the
		// margins)

		boardImage = new BufferedImage((WIDTH + 2) * SIZE, (HEIGHT + 2) * SIZE, BufferedImage.TYPE_INT_RGB);
...
//--------- generate the Hills --------------------------------------------
		...
		for (int i = 0; i < numberOfHills; i++) // for each element in the array,
		{
			do {
				int x = 2 * SIZE + rand.nextInt(SIZE * (WIDTH - 2)), // determine the position
						y = 2 * SIZE + rand.nextInt(SIZE * (HEIGHT - 2)), d = rand.nextInt(sizeMax - sizeMin) + sizeMin, // and
																															// diameter
	...
	// ----------------Checks if position is on the
	// board----------------------------------

	boolean isLegal(Position p) {
		return (p != null) && boardArea.contains(p.x, p.y);
	} // A position is legal if non null and on the board
	...
	float elevation(Position pos) {
		for (Hill hill : hills) // for every hill
			if (hill.onIt(pos))
				return hill.height; // if on it then return the hill height
	...
	Piece cycleSelector() {
		...
		if (selectorIdx == 4)
			...
			y = selectorIdx + 1; // going from 2 to 6
		while (y != selectorIdx) {
		...
		if (initialselectors[y].shouldShowSelector()) // nothing found on one trip
			...
			return null; // so let previous selector remain if possible
	}

	// ----------------Find selector at position
	// p-----------------------------------------
	...
	// If so, we return the position of that piece. This means when you click a
	// piece you don't have to
	// hit the exact center. An exception is when moving a piece only a very short
	// distance, then the mouse release will happen when it is still very close to
	// itself.
	// Therefore an extra parameter of type Piece tells what piece should be exempt
	// from
	...
	{ // the piece exempt from proximity check
		Position res = new Position(x, y); // First create the proper position
		{
			...
			for (Piece thePiece : pieces) { // It may be "on" several pieces
				if (thePiece.on(res) // if so we must find the closest to res
						& !(thePiece == exemptPiece) & res.distance(thePiece) < min) {
	...
	Position realPosition(int x, int y) {
		return (realPosition(x, y, (Piece) null));
	} // Ditto, without info about exempt piece
	...
	Piece addPiece(Piece p) {
		...
		if (victim != null)
			removePiece(victim); // then remove the piece there
	...
	Piece find(Position pos) {
		...
		for (Piece piece : pieces) // For all pieces
			if (piece.pos.equals(pos))
				result = piece; // if its position is pos then it is it.
		return result;
	}
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...

	{
		...
		// This will paint the buffered image of the board, omitting current position
		// marker, clock, and selectors

		BufferedImage bi = new BufferedImage((WIDTH + 2) * SIZE, (HEIGHT + 2) * SIZE, // get a new temporary image to
																						// draw on
				BufferedImage.TYPE_INT_RGB);
		Graphics2D g = bi.createGraphics();
		g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);

		// Begin by drawing the background, a board where everything is invisible, and
		// frame
		...
		{
			if (thePiece == currentPiece)
				thePiece.draw(g, Color.red); // Currently grabbed piece is red
...
//------------  Show who controls most area
		...
		g.setFont(myFont);
		if (relativeNumberOfControlledSquares < 0)
...
// --------------- Show if I can move
		...
		if (!waiting & !playBackBoard)
			myPebble.draw(page, (new Position(SIZE / 2, SIZE * (HEIGHT + 1))), Color.red);
...
// ---------------- Show clock
		...
		page.drawString(clock / 60 + ":" + (clock % 60) / 10 + (clock % 60) % 10, (SIZE * WIDTH + SIZE / 2), SIZE / 2);
...
// ------------------ show stall message
	...
	// -------------------------------------------------
	// A piece is uncontested if its range does not intersect the range of any
	// opposite piece
	// -------------------------------------------------
	...
	private boolean uncontested(Piece p) {
		...
		for (Piece other : pieces) // piece, call it other
		{
			if ((p.myside != other.myside) // playing for the opposite side
					&& p.currentRange + other.currentRange >= p.pos.distance(other)) // and so close that the range
																						// areas overlap
				res = false;
		}
		return res;
	...
	// During the recalculation of control areas the board is partitioned into
	// regions. The regions are formed
	// by intersections of the piece range areas.
	// Each region has an Area which it occupies, an integer control value which is
	// the amount of control
	// exerted on it so far (positive for my side, negative for opponent, zero for
	// neutral), a boolean visibility
	// indicator which tells if that region should be painted on the board, and
	// Rectangle bounding box.
	// The reason for the latter is that it is more efficient to perform some of the
	// computations on
	// bounding boxes before emabarking on more time consuming calculations on
	// Areas.
	...
	private class Region {
		...
		float radius; // radius of the bounding box. So all positions in the area is within radius of
						// the center.
		...
		private boolean decided(ArrayList<Piece> pieces) // parameter is opponent pieces to take into consideration
		{
			...
			for (Piece inspected : pieces) { // as long as c is positive, and for all pieces
				if (center.distance(inspected) <= inspected.currentRange + radius) // if it is so close that it might
																					// have an influence on my area
					c -= inspected.power; // decrease c by its power
			}
			return c > 0; // return true iff c is still positive
		}

		// ------------- addMe: add me to a list of regions unless my fate can already
		// be inferred

		// The list of regions is the list of still undecided regions who need further
		// examination
		//
		...
		private void addMe(ArrayList<Region> regList, ArrayList<Piece> pieces) {
			if (control < 0) // if control <0 then
			{
				...
				if (debug)
					debugRegions.add(this); // add it to his area and
				if (visible)
					opponentVisibleArea.add(area);
			} // not to the region list
			else // if control >= 0
			if (decided(pieces)) // and the area is deceidedly mine
			{
				myArea.add(area);
				if (debug)
					debugRegions.add(this);
			} // then add it to my area
			else
				regList.add(this); // else add it to the list of regions
		}

		// ------------- checkPiece--------------------------
		//
		// This determines what happens to the me when a Piece is taken into
		// consideration.
		// If the range area completely encloses me, then update the control value with
		// the amount
		// exerted by the piece. If the range area is disjoint from me then do nothing.
		// If it intersects me
		// then create a new region corresponding to the intersection, and subtract it
		// from my area. For
		...
		// parameter is a list of enemy pieces to consider when determining if the
		// region is decided.

		private void checkPiece(Piece p, ArrayList<Region> newRegions, ArrayList<Piece> pieces) {
			if (center.distance(p) > radius + p.currentRange)
				newRegions.add(this); // area completely out of range: just forward the area to newRegions
			else if (p.rangeArea.contains(boundingBox)) // If my bounding box lies completely in range
			{
				control += p.power * (p.myside ? 1 : -1);
				numberOfBox++;// update my control value with the power of the piece
				addMe(newRegions, pieces);
			} // and add me to the new regions
			else if (p.rangeArea.intersects(boundingBox)) // else, if the bounding box intersects the range
			{
				...
				else if (!newArea.isEmpty()) // else, if that turns out to be nonempty
				{
					...
					addMe(newRegions, pieces); // then just forward me to new Regions
	...
	// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	// calculateControl: determine myArea, opponentArea, opponentVisibleArea,
	// onAreas. Also count number of pieces on board
	// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...

	{
		...
		// Let uncontested pieces exert directly (these will not need to generate
		// regions)
		...
		for (Piece inspected : pieces) {
			if (inspected.myside) // if it is my piece
			{
				...
				{
					if (!uncontested(inspected))
						myPieces.add(inspected); // add it to the list of my pieces
					else {
						myArea.add(inspected.rangeArea); // or, if uncontested, add the range area directly
						numberOfUncontested++;
					}
				}
			...
			{
				if (!uncontested(inspected))
					opponentPieces.add(inspected); // and add it to the list of opponent pieces
				else // or, if uncontested
				{
					opponentArea.add(inspected.rangeArea);
					numberOfUncontested++;
					if (allVisible)
						opponentVisibleArea.add(inspected.rangeArea); // add it to opponentArea or opponentVisibleArea
				}
			}
		} // end for each piece
		...
		// ----------- check the influence of all my contested pieces on the control
		// areas
		//
		// This will generate regions of increasing positive control, as the range areas
		// intersect.
		...
		for (Piece inspected : myPieces) // for each of my pieces
		{
			...
			for (Region thisRegion : regions) // for each existing region
				thisRegion.checkPiece(inspected, newRegions, opponentPieces); // let the region check the piece for
																				// possible influence on its area and
																				// control // end for all regions
		...
		while (!opponentPieces.isEmpty()) // for each of opponent's pieces
		{
			...
			opponentPieces.remove(inspected); // remove it from opponentPieces (so it cannot unduly affect the screening
												// by "decided")
		...
		for (Region thisRegion : regions) {
			if (thisRegion.control > 0)
				myArea.add(thisRegion.area);
			if (thisRegion.control == 0 & thisRegion.visible)
				neutralArea.add(thisRegion.area);
			if (thisRegion.control < 0 & thisRegion.visible)
				opponentVisibleArea.add(thisRegion.area);
			if (thisRegion.control < 0)
				opponentArea.add(thisRegion.area);
		}
	...
	// the algorithm is to generate a grid of points (of cardinality
	// numberOfSamples^2)
...

	{
		...
		final float scaleFactor = numberOfSamples * numberOfSamples / HEIGHT / WIDTH; // to scale the result down to
		...
		for (float i = SIZE; i <= SIZE * (WIDTH + 1); i = i + sampleIncrementX) // so just check all sample points
			for (float j = SIZE; j <= SIZE * (HEIGHT + 1); j = j + sampleIncrementY) {
				if (myArea.contains(i, j))
					relativeNumberOfControlledSquares++;
				else if (opponentArea.contains(i, j))
					relativeNumberOfControlledSquares--;
			}
		relativeNumberOfControlledSquares = Math
				.round(((float) (relativeNumberOfControlledSquares) / (float) scaleFactor));
	}
	...
	void calculateHighlight(Piece currentPiece) {
		...
		if (currentPiece != null) // only if there is a current piece
		{
			...
			for (Piece inspected : pieces) // For each enemy piece
				if (!inspected.myside & highlightArea.contains(inspected.pos.x, inspected.pos.y)) // that sits in this
																									// area
					inspected.underAttack = true; // set the underAttack flag

			for (Piece inspected : pieces) // For each piece except the currentPiece
				if (inspected != currentPiece)
					highlightArea.subtract(inspected.avoidArea(currentPiece)); // subtract the avoidArea.
		} else
			highlightArea.reset(); // no current piece: higlightArea is null
...
{
	// ----------- The following parameters uniquely determine the piece type.
	// Change them when devising new pieces.
	...
	final static private Color WHITE_NEUTRAL = new Color(210, 210, 210); // Various colors of pieces when not controlled
																			// by owner
	...
	static Piece newPiece(Type pt, boolean w, boolean my, Board.Position p, Board b) // create new piece of right type
	{
		...
		switch (pt) {
		case PEBBLE:
			res = new Pebble(w, my, p, b);
			break;
		case RUBBLE:
			res = new Rubble(w, my, p, b);
			break;
		case QUORUM:
			res = new Quorum(w, my, p, b);
			break;
		case BOUNCER:
			res = new Bouncer(w, my, p, b);
			break;
		case NIMBLER:
			res = new Nimbler(w, my, p, b);
			break;
		case KEEP:
...
//--------------------------------------------------------------------
	...
	Piece(boolean w, boolean my, Board.Position pos, Board board) {
		...
		if (board.shape == 2)
			...
			nPoly = 28; // Determine polygon degree from shape param
	...
	// ----------------------------------------------------------------------------------
	// circArea: generate an area around pos with given radius. if isCirc it is a
	// perfect circle, OW as determined by shape
	// ----------------------------------------------------------------------------------
	...
	private Area circArea(int radius, boolean isCirc) {
		...
		if (isCirc)
			circ = new Ellipse2D.Float((int) (pos.x - radius), (int) (pos.y - radius), (int) (2 * radius),
					(int) (2 * radius));

		// shape == 1: generate square

		else if (board.shape == 1)
			circ = new Rectangle((int) (pos.x - radius / Math.sqrt(2)), (int) (pos.y - radius / Math.sqrt(2)),
	...
	// ---------------------------------------------------------------------
	// Draw a piece type with specified color, including a bounding outline of
	// opposite color
	// ---------------------------------------------------------------------
	...
	// ------------------------------------------------------------------------------
	// Default canBuild, determines if a piece of this type can be built at position
	// p.
	// refined by subclasses. side = true means it is me who builds, ow opponent
	// builds
	// -------------------------------------------------------------------------------
...
class Pebble extends Piece {
	...
	final private Keep myKeep = new Keep(true, true, null, board); // A Keep, just to be able to show an outline when
																	// promoting a rubble

	// -------------------------------------------------------------------
	// Construct a pebble: construct a Piece, set its type to pebble, and decrease
	// remaining pebbles
	// -------------------------------------------------------------------
	...
	Pebble(boolean w, boolean my, Board.Position pos, Board board) {
		...
		if (board.isLegal(pos))
			board.decreaseRemaining(w); // decrease remaining pebbles if on the board
	} // (ow it is a selector)
	...
	{
		return new Area(new Ellipse2D.Float(pos.x - OSIZE / 2, pos.y - OSIZE / 2, OSIZE, OSIZE));
	}
	...
	@Override
	boolean canBuild(Board.Position p) {
		return board.canBuildPebbleOn(p);
	} // Builds possible on buildable area

	// ----------------------------------------------------------------------
	// Effects of moving a pebble (overrides Piece.moveTo because this can create a
	// keep
	// ----------------------------------------------------------------------
	...
	Piece moveTo(Board.Position pos) {
		...
		if (dest == null || dest.pieceType != Type.RUBBLE || dest.myside != myside) // An ordinary move
			...
			board.removePiece(dest); // but a keep-building move
	...
	boolean canMove(Board.Position p) {
		...
		return super.canMove(p) || canKeep(p);
	} // Either an ordinary move or a keep build
	...
	boolean canKeep(Board.Position p) {
		if (board.isLite)
			return false; // In a lite game you can never build a Keep
		else {
			Piece inhabitant = board.find(p);
			...
			if (res)
				outline = myKeep; // change outline if I can build a keep
			return res;
		}
	}
...
class Rubble extends Piece {
	...
	{
		return new Area(new Rectangle2D.Float(pos.x - RSIZE / 2, pos.y - RSIZE / 2, RSIZE, RSIZE));
	}
	...
	{
		return board.howManyOnBoard(Type.PEBBLE) > 0;
...
class Keep extends Piece {
	...
	final private int[] Kx = { KM, BSIZE / 2 - KW / 2, BSIZE / 2 - KW / 2, BSIZE / 2 + KW / 2, BSIZE / 2 + KW / 2,
			BSIZE - KM, BSIZE - KM, BSIZE / 2 + KW / 2, BSIZE / 2 + KW / 2, BSIZE / 2 - KW / 2, BSIZE / 2 - KW / 2,
			KM };
...
abstract class HeavyPiece extends Piece {
	...
	boolean specificSelector() {
		return board.howManyOnBoard(Type.KEEP) > 0 && board.howManyOnBoard(Type.PEBBLE) > 0;
	} // same for all heavy pieces
...
class Bouncer extends HeavyPiece {
	...
	Area appearance(Board.Position pos) {
		Area app = new Area(new Ellipse2D.Float(pos.x - OSIZE / 2, // Appearance is a circle plus two polygons
				pos.y - OSIZE / 2, OSIZE, OSIZE));
		int[] Bx1 = { pos.x - BSIZE / 2, pos.x + BSIZE / 2 - BWIDTH / 2, pos.x + BSIZE / 2,
				pos.x - BSIZE / 2 + BWIDTH / 2 };
		int[] By1 = { pos.y - BSIZE / 2 + BWIDTH / 2, pos.y + BSIZE / 2, pos.y + BSIZE / 2 - BWIDTH / 2,
				pos.y - BSIZE / 2 };
		app.add(new Area(new Polygon(Bx1, By1, 4)));
		int[] Bx2 = { pos.x + BSIZE / 2 - BWIDTH / 2, pos.x - BSIZE / 2, pos.x - BSIZE / 2 + BWIDTH / 2,
				pos.x + BSIZE / 2 };
		int[] By2 = { pos.y - BSIZE / 2, pos.y + BSIZE / 2 - BWIDTH / 2, pos.y + BSIZE / 2,
				pos.y - BSIZE / 2 + BWIDTH / 2 };
...
class Quorum extends HeavyPiece {
	...
	{
		Area app = new Area(new Ellipse2D.Float(pos.x - OSIZE / 2, pos.y - OSIZE / 2 + OSIZE / 4, OSIZE, OSIZE));
		app.add(new Area(new Ellipse2D.Float(pos.x - OSIZE / 2, pos.y - OSIZE / 2 - OSIZE / 4, OSIZE, OSIZE)));
		return app;
	}
	...
	protected void drawOutline(Graphics page, Board.Position pos, Color color) {
		...
		page.drawOval(pos.x - OSIZE / 2, pos.y - OSIZE / 2 + OSIZE / 4, OSIZE, OSIZE);
	}
...
class Nimbler extends HeavyPiece {
	...
	// ---------------------------------------------------------------------
	Area appearance(Board.Position pos) {
		Area app = new Area(new Ellipse2D.Float(pos.x - OSIZE / 2, pos.y - OSIZE / 2, OSIZE, OSIZE));
		for (int i = 0; i <= 5; i++) {
			Nxx[i] = Nx[i] + pos.x;
			Nyy[i] = Ny[i] + pos.y;
		}
