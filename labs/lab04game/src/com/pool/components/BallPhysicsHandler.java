...
public class BallPhysicsHandler extends Component {
	...
	private boolean checkBallToBallCollissions() {

		// We store all collisions in a list, to avoid checking the same collision
		// twice. Each arraylist entry is an array of two balls that have collided.
		...
		for (GameObject ball : balls) {
			for (GameObject other : balls) {
				...
				if (isPickedUpQueueBall(ball) || isPickedUpQueueBall(other))
					continue;
		...
		// When we know all collisions on the table, we can handle them at the "same"
		// time.
		handleCollissions(collissions);

		return collissions.size() > 0;
	}

	private boolean isPickedUpQueueBall(GameObject ball) {
		BallSprite bs = (BallSprite) ball.getComponent(BallSprite.class);
		if (!bs.isQueueBall())
			return false;
		return ((QueueBallBehaviour) ball.getComponent(QueueBallBehaviour.class)).isOffSide();
	}

	private void checkBallToWallCollissions() {
		for (GameObject ball : balls) {
			if (isPickedUpQueueBall(ball))
				continue;

			// Retrieve the rigidbody component of the ball, and if there is a wall
			// collision, reverse the appropriate speed axis.
	...
	public boolean canPlaceQueueBall(Vector pos) {
		for (GameObject ball : balls) {
			BallSprite bs = (BallSprite) ball.getComponent(BallSprite.class);
			if (bs.isQueueBall()) continue;
			
			if (Physics.ballsCollide(ball.position, pos)) return false;
		}
		
		if (Physics.leftWallCollission(pos.x, leftWallX, -1)) return false;
		if (Physics.rightWallCollission(pos.x, rightWallX, 1)) return false;
		if (Physics.topWallCollission(pos.y, topWallY, -1)) return false;
		if (Physics.bottomWallCollission(pos.y, bottomWallY, 1)) return false;
		return true;
	}
